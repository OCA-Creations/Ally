...
struct DotFile: Codable {
    /// The location of the dotfile to be parsed.
    var location: URL
    ...
    func output(docs: Bool) {
        ...
        for alias in self.aliases {
            // The variable `docs` is whether to print out docs
            // We unwrap the docs of the alias to `theDocs` from i.docs
            if docs, let theDocs = alias.docs {
                    boldPrint(theDocs)
            }
            boldPrint("\(alias.aliasName)", terminator: " -> ")
            print(alias.longFormCommand)
        }
    }
}
